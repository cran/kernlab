...
The \texttt{promotergene} data set is a data set of 
...
\item[\texttt{kernelFast}]
This method is different to \texttt{kernelMatrix} for \texttt{rbfdot}, \texttt{besseldot}, 
and the \texttt{laplacedot} kernel, which are all RBF kernels. 
It is identical to \texttt{kernelMatrix}, 
...
evaluating the norm of each column-entry as it is done on a \texttt{kernelMatrix} 
...
In order to create a similar probability output for regression, following 
\citep{kernlab:Weng:2004}, we suppose that the SVM is trained on data from the model 
\begin{equation}
y_i = f(x_i) + \delta_i
\end{equation}
...
An empty object of class \texttt{onlearn} is initialized using the \texttt{inlearn} function. 
<<<onlearn, echo = TRUE>>=
## create toy data set
...
Kernel feature analysis \citep{kernlab:Olvi:2000} is computationaly simple and scales approximately
 one order of magnitute better on large data sets then standard KPCA.    
...
orthogonal subspaces. Due to the $l_1$ constrain the solution has the favorable property of beeing 
sparse in terms of the coefficients $\alpha_i$.

The function \texttt{kfa} in \pkg{kernlab} implements Kernel Feature Analysis by using a projection 
pursuit technique on a sample of the data. Results are then returned in an \textsf{S4} object. 
