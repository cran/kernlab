...
%\usepackage{alltt,ARd,A}
\usepackage{alltt,hyperref,a4wide}
...
%\VignetteKeywords{kernel methods, support vector machines, online algorithms, quadratic optimizer, kernel PCA, R}
...
\title{\pkg{kernlab} -- An \textsf{S4} package for kernel methods in
  \textsf{R}}
\author{Alexandros Karatzoglou
  \thanks{Institut f\"ur Statistik \& Wahrscheinlichkeitstheorie,
    Technische Universit\"at Wien, Austria}
  \and
  Alex Smola
  \thanks{Australian National University, Department of Engineering and
    RSISE, Australia}
  \and
  Kurt Hornik
  \thanks{Institut f\"ur Statistik und Mathematik,
...
\begin{abstract}
  ...
  learning methods in
  \textsf{R}\footnote{\url{http://www.r-project.org/}}.  It takes
  advantage of \textsf{R}'s new \textsf{S4} object model and provides a
  ...
  a ranking algorithm, kernel PCA, kernel CCA, and a spectral clustering
  algorithm.  Moreover it provides a general purpose quadratic
  programming solver, and an incomplete Cholesky decomposition method.
\end{abstract}

\noindent
\textbf{ Keywords:} kernel methods, support vector machines, quadratic
programming, ranking, clustering, \textsf{S4}, \textsf{R}.
...
clustering
%% ---or more generally: supervised and unsupervised learning---
...
software is written in \textsf{C} or \textsf{C++}, e.g.\ the award
winning
\texttt{libsvm}\footnote{\url{http://www.csie.ntu.edu.tw/~cjlin/libsvm/}}
\citep{kernlab:Chang+Lin:2001},
\texttt{SVMlight}\footnote{\url{http://svmlight.joachims.org}}
\citep{kernlab:joachim:1999},
\texttt{SVMTorch}\footnote{\url{http://www.torch.ch}}, Royal Holloway
Support Vector Machines\footnote{\url{http://svm.dcs.rhbnc.ac.uk}},
\texttt{mySVM}\footnote{\url{http://www-ai.cs.uni-dortmund.de/SOFTWARE/MYSVM/index.eng.html}},
and \texttt{M-SVM}\footnote{\url{http://www.loria.fr/~guermeur/}} with
many packages providing interfaces to \textsf{Matlab} (such as
\texttt{libsvm}), and even some native \textsf{Matlab}
toolboxes\footnote{
...
\texttt{Spider}\footnote{\url{http://www.kyb.tuebingen.mpg.de/bs/people/spider/}}
software which provides a \textsf{Matlab} interface to various
\textsf{C}/\textsf{C++} SVM libraries and \textsf{Matlab}
implementations of various kernel-based algorithms,
Torch\footnote{\url{http://www.torch.ch}} which also includes more
traditional machine learning algorithms, and the occasional
\textsf{Matlab} or \texttt{C} program found on a personal web page where
an author includes code from a published paper.

\subsection{R Software}

The \pkg{e1071} \textsf{R} package offers an interface to the award
winning \texttt{libsvm} \citep{kernlab:Chang+Lin:2001}, a very efficient
SVM implementation.  \texttt{libsvm} provides a robust and fast SVM
implementation and produces state of the art results on most
classification and regression problems
\citep{kernlab:Meyer+Leisch+Hornik:2003}.  The \textsf{R} interface
provided in \pkg{e1071} adds all standard \textsf{R} functionality like
object orientation and formula interfaces to \texttt{libsvm}.  Another
SVM related \textsf{R} package which was made recently available is
\pkg{klaR} \citep{kernlab:Roever:2004} which includes an interface to
\texttt{SVMlight}, a popular SVM implementation along with other
classification tools like Regularized Discriminant Analysis.

However, most of the \texttt{libsvm} and \pkg{klaR} SVM code is in
\textsf{C++}.  Therefore, if one would like to extend or enhance the
code with e.g.\ new kernels or different optimizers, one would have to
modify the core \textsf{C++} code.

\section[kernlab]{\pkg{kernlab}}

\pkg{kernlab} aims to provide the \textsf{R} user with basic kernel
...
\subsection{\textsf{S4} objects}

\pkg{kernlab} uses \textsf{R}'s new object model described in
``Programming with Data'' \citep{kernlab:Chambers:1998} which is known
as the \textsf{S4} class system and is implemented in the
\pkg{methods} package.

In contrast with the older \textsf{S3} model for objects in \textsf{R},
classes, slots, and methods relationships must be declared explicitly
when using the \textsf{S4} system.  The number and types of slots in an
...
definition and have to comply to it at any time.  \textsf{S4} also
requires formal declarations of methods, unlike the informal system of
using function names to identify a certain method in \textsf{S3}.

An \textsf{S4} method is declared by a call to \texttt{setMethod} along
...
Generic functions can be declared using the \texttt{setGeneric}
function.  Although such formal declarations require package authors to
be more disciplined then when using the informal \textsf{S3} classes,
...
Namespaces were introduced in \texttt{R} 1.7 and provide a means for
...
made visible outside the package.  Since \textsf{S4} methods are being
used, the \pkg{kernlab} namespace also imports methods and variables
form the \pkg{methods} package.

\subsection{Data}

The \pkg{kernlab} package also includes data sets which will be used
to illustrate the methods included in the package.  The \texttt{spam}
data set \citep{kernlab:Hastie:2001} set collected at Hewlett-Packard
Labs classifies 4601 e-mails as spam or non-spam.  The 57 variables of
each data vector indicate the frequency of certain words and characters
in the e-mail.  The data set contains 2788 and 1813 e-mails classified
as non-spam and spam, respectively.

Another data set included in \pkg{kernlab} the \texttt{income} data
...
The \texttt{ticdata} data set \citep{kernlab:Putten:2000} was used in
...
The \texttt{promotergene} data set is a data set with 106 observations and 58 
variables. Promoters have a region where a protein (RNA polymerase) must make 
...
The \texttt{spirals} data set was created by the
\texttt{mlbench.spirals} function in the \pkg{mlbench} package
...
Kernels in \pkg{kernlab} are \textsf{S4} objects of class
\textsf{kernel} extending the \textsf{function} class with one
...
contain the class \textsf{kernel} and are used to implement the existing
...
\begin{itemize}
 \item the linear ``vanilladot'' kernel implements the simplest of all
  kernel functions
  \begin{equation}
 ...
 \item the Gaussian radial basis function ``rbfdot''
  \begin{equation}
 ...
 \item the polynomial kernel ``polydot''
  \begin{equation}
    k(x, x') = 
    \left(
 ...
 \item the hyperbolic tangent kernel ``tanhdot''
  \begin{equation}
    k(x, x') = 
    \tanh 
    \left(
 ...
 \item the Bessel function of the first kind kernel ``besseldot''
  \begin{equation}
    k(x, x') = 
 ...
 \item the Laplace radial basis kernel ``laplacedot''
  \begin{equation}
    k(x, x') = \exp(-\sigma \|x - x'\|)
  \end{equation}
  which is a general purpose kernel and is typically used when no
 ...
 \item the ANOVA radial basis kernel ``anovadot''
  \begin{equation}
    k(x, x') = \left(\sum_{k=1}^{n}\exp(-\sigma(x^k-y^k)^2)\right)^{d}
  \end{equation}
...
function through a pair of \textsf{for} loops.  The kernel methods
included are:

\begin{description}
  
 \item[\texttt{kernelMatrix}] This is the most commonly used function.
...
\end{verbatim}
 ...
 \item[\texttt{kernelMult}] is a convenient way of computing kernel
  ...
  \texttt{blocksize} determines the number of rows in the stripes.  In
  particular,
\begin{verbatim}
...
\end{verbatim}
 ...
 \item[\texttt{kernelPol}]
  is a method very similar to \texttt{kernelMatrix} with the only
...
\end{verbatim}
  ...
  \texttt{x1} and \texttt{x2} may be matrices and \texttt{y1} and
  \texttt{y2} vectors.
\end{description}
...
wider range of kernel-based methods in \textsf{R}.  In the rest of the
...
some criterion over the sample.  In the case of the 1-norm Soft Margin
...
\texttt{ksvm}, is based on the optimizers found in
\texttt{bsvm}\footnote{\url{http://www.csie.ntu.edu.tw/~cjlin/bsvm}}
\citep{kernlab:Hsu:2002} and \texttt{libsvm}
\citep{kernlab:Chang+Lin:2001} which includes an very efficient version
...
problem involving two elements of $\alpha_i$ because the must obey one
...
The SVM implementations available in \texttt{ksvm} include the C-SVM
...
For classification problems which include more then two classes
...
\texttt{ksvm} implementation provides the ability to produce class
...
\texttt{couple} method which implements methods for combing class
probabilities proposed in \citep{kernlab:Wu:2003}.

In order to create a similar probability output for regression 
we suppose that the SVM is trained on data from the model 
\begin{equation}
y_i = f(x_i) + \delta_i
\end{equation}
...
Another approach for multi-class classification supported by the
\texttt{ksvm} function is the one proposed in
\citep{kernlab:Crammer:2000}.  This algorithm works by solving a single
...
in \citep{kernlab:Hsu:2002} where optimal working sets are found (that
...
the
\texttt{TRON}\footnote{\url{http://www-unix.mcs.anl.gov/~more/tron/}}
...
\texttt{ksvm}.  SVM novelty detection works by creating a spherical
...
quantile of the $\|x- x'\| $ statistics.  The \texttt{sigest} function
...
An example for the \texttt{ksvm} function is shown below. 
...
\begin{figure}
\centering
<<fig=TRUE,echo=TRUE,width=10,height=10>>=
x <- rbind(matrix(rnorm(120),,2),matrix(rnorm(120,mean=3),,2))
y <- matrix(c(rep(1,60),rep(-1,60)))

svp <- ksvm(x,y,type="C-classification")
plot(svp)
@  
\caption{A contour plot of the SVM decision values for a toy binary classification problem using the plot function}
...
type~II maximum likelihood method for which can be used for regression.
The functions returns an \textsf{S4} object containing the model
...
For regression estimation we assume that rather then observing $t(x_i)$
we observe $y_i = t(x_i) + \xi_i$ where $\xi_i$ is assumed to be
independed Gaussian distributed noise with zero mean.  The posterior
...
\texttt{gausspr} is the function in \pkg{kernlab} implementing Gaussian
...
and the point are ranked according to the scores they received.

Suppose we are given a set of data points $X = {x_1, \dots, x_{s},
  x_{s+1}, \dots, x_{m}}$ in $\mathbb{R}^n$ where the first $s$ points
...
\pkg{kernlab} includes an \textsf{S4} method implementing the ranking
...
\caption{The points on the left are ranked according to their similarity
  ...
  then points in the lower structure.}
\label{fig:Ranking}
\end{figure}

\subsection{Online Learning with Kernels}

The \texttt{onlearn} function in \pkg{kernlab} implements the online kernel algorithms 
for classification, novelty detection and regression descibed  in \citep{kernlab:Kivinen:2004}.
...
\end{equation}  
Since usually $P$ is unkown a standart approach is to instead minimize the empirical risk 
\begin{equation}
...
data but do not generalise to unseen data). One way to avoid this is to penalise complex functions by 
instead minimizing the regularised risk.
\begin{equation}
...
of an instanteneous regularised risk on a single example is needed
\begin{equation}
...
The \texttt{onlearn} function implements the online learning algorithm for regression, classification and novelty 
detection. The online nature of the algorith requires a differnt aproach to the use of the function. An object 
is used to store the state of the algorithm at each iteration $t$ this object is passed to the function ans an argument and
is returned at each  iteration $t+1$ containing the model paramemeter state at this step. 
An empty object is initialized using the \texttt{inlearn} function. 
...
\texttt{kmeans} is finally used to cluster the points.

\pkg{kernlab} includes an \textsf{S4} method called \texttt{specc}
implementing this algorithm which can be used through an formula
interface or a matrix interface.  The \textsf{S4} object returned by the
...
plot(spirals, col=sc)
@  
\caption{Clustering the two spirals data set with \texttt{specc}}
...
$x_k \in \mathbb{R}^N$, PCA diagonalizes the covariance matrix $C =
...
nonlinear map $\Phi:\mathbb{R}^N \rightarrow F$, $x \mapsto \mathbf{X}$.
...
\texttt{kpca}, the the function implementing KPCA in \pkg{kernlab}, can
be used both with a formula and a matrix interface, and returns an
\textsf{S4} object of class \texttt{kpca} containing the principal
...
\texttt{predict} function can be used to embed new data points into the
...
Sparse solutions are often achieven in supervised learning settings by using an $l_1$ penalty on the 
expansion coefficients. An algorithm can be derived using the same approach in feature extraction requiring only $n$ 
basis functions to compute the first $n$ feature.
Kernel feature analysis is computationaly simple and scales approximately one order of magnitute
better on large data sets then standard KPCA.    
...
Subsequent ``principal vectors'' can be defined by enforcing optimality with respect to the remaining orthogonal subspaces.
Due to the $l_1$ constrain the solution has the favorable property of beeing sparse in terms of the coefficients $\alpha_i$.

The function \texttt{kfa} in \pkg{kernlab} implements Kernel Feature Analysis by using a projection pursuit technique on a 
sample of the data. Results are then returned in an \textsf{S4} object. 
...
$\Phi:\mathbb{R}^N \rightarrow F$, $x \mapsto \mathbf{X}$ as
\begin{eqnarray*}
...
\texttt{kcca} method in \pkg{kernlab} returns an \textsf{S4} object
...
\pkg{kernlab} provides the \textsf{S4} method \texttt{ipop} implementing
...
\texttt{inc.chol} which computes the decomposed matrix $\tilde{Z}$ from
...
kernel methods package for \textsf{R} with existing modern kernel
...
kernel-based algorithms in \textsf{R} while using all of \textsf{R}'s
modern facilities, like \textsf{S4} classes and namespaces.  Our aim for
