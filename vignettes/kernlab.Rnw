...
on <- inlearn(2,kernel="rbfdot",kpar=list(sigma=25),type="classification")
ind <- sample(1:90,90)
...
## initialize onlearn object
on <- initOLK(2,kernel="rbfdot",kpar=list(sigma=10),type="classification")
ind <- sample(1:90,90)
## learn one data point at the time
for(i in ind)
on <- olk(on,x[i,],y[i], C = 0.25, r = 10^(-5), epsilon = 0.01)
sign(predict(on,x))
@ 
