...
\begin{figure}
\centering
<<echo=TRUE,fig.width=10,fig.height=10,results='hide'>>=
set.seed(123)
...
\begin{figure}
\centering
<<ranking,echo =TRUE,fig.width=12,fig.height=7>>=
data(spirals)
...
on <- inlearn(2,kernel="rbfdot",kpar=list(sigma=1.8),type="classification")
ind <- sample(1:90,90)
## learn one data point at the time
for(i in ind)
on <- onlearn(on,x[i,],y[i],nu=0.4,lambda=0.5,eta=0.001)
sign(predict(on,x))
@ 

\subsection{Model Based Online learning with kernels}

The \code{olk} function in \pkg{kernlab} implements the OLK algorithm 
for classification, novelty detection and regression described in \citep{kernlab:li2013model}.
OLK proposes to solve the following:
\begin{equation}
  \begin{array}{ll}
    \min\limits_{f,\xi_{t+1}} & \frac{1}{2}\|f-f_t\|^2 + \frac{r}{2}\|f\|^2 + C\xi_{t+1} \\
    s.t. & y_{t+1}f(x_{t+1}) \geq 1 - \xi_{t+1}, \xi_{t+1} \geq 0
  \end{array}
\end{equation}  
The solution is the summation of the weighted kernel function as follows:
\begin{equation}
f(x_t) = \sum\limits \alpha_i k(x_i, x_t)
\end{equation}
Each of the three applications (classification, novelty detection and regression) have
a different calculation for $\alpha$ available in \citep{kernlab:li2013model}. The parameter $C$
is the soft margin similar to the SVM. $r$ is the forgetting rate where the weights will decay
each time a example is added by $\alpha = \frac{\alpha}{1+r}$. This should typically be small
around $10^{-3}$ or $10^{-4}$. epsilon is only used in regression. It is a error tolerance parameter.
If the prediction for a given example is within epsilon of $y_{t+1}$ the example is not added to the
dictionary of examples.

The \code{olk} function implements the online learning algorithm for regression, classification and novelty 
detection. The online nature of the algorithm requires the same approach as \code{onlearn}. An object 
is used to store the state of the algorithm at each iteration $t$ this object is passed to the function as an 
argument and is returned at each iteration $t+1$ containing the model parameter state at this step. 
An empty object of class \code{olk} is initialized using the \code{initOLK} function. 
<<olk, echo = TRUE>>=
## create toy data set
x <- rbind(matrix(rnorm(90),,2),matrix(rnorm(90)+3,,2))
y <- matrix(c(rep(1,45),rep(-1,45)),,1)
...
## initialize onlearn object
on <- initOLK(2,kernel="rbfdot",kpar=list(sigma=2.5),type="classification")
ind <- sample(1:90,90)
## learn one data point at the time
for(i in ind)
on <- olk(on,x[i,],y[i], C = 0.25, r = 10^(-5), epsilon = 0)
sign(predict(on,x))
@ 
...
\begin{figure}
\centering
<<kpca,echo =TRUE,fig.width=7,fig.height=7>>=
data(spam)
...
\begin{figure}
\centering
<<kfa, echo = TRUE,fig.width=7,fig.height=7>>=
data(promotergene)
...
plot(predict(f,promotergene),col=as.numeric(promotergene[,1]),
     xlab="1st Feature",ylab="2nd Feature")
@ 
